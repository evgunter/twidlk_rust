...
type UsbHidKey = (Shifted, u8);
type UsbHidValue = (Shifted, String);

lazy_static! {
    static ref USB_HID_TABLE: Vec<(UsbHidKey, UsbHidValue)> = {
    ...
    static ref USB_HID_MAP: HashMap<UsbHidKey, UsbHidValue> = 
        USB_HID_TABLE.iter().cloned().collect();

    static ref USB_HID_UNMAP: HashMap<String, (Shifted, Shifted, u8)> = {
...
fn read_config(contents: &[u8]) -> std::io::Result<TwiddlerConfig> {
...
fn read_text_config(contents: &str) -> TwiddlerConfig {
...
fn main() -> std::io::Result<()> {
    ...
    let input_is_text = contents.starts_with(b"version 0\n");

    let config = if input_is_text {
        read_text_config(std::str::from_utf8(&contents).unwrap())
    } else {
        read_config(&contents).unwrap()
    };

    let new_extension = if input_is_text { "cfg" } else { "txt" };
    let new_filename = Path::new(filename).with_extension(new_extension);

    if new_filename.exists() {
        let source_modification_time = fs::metadata(filename)?.modified()?;
